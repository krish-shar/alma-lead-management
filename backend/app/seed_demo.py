...
PERSONAS = [
    ...
    ("Mateo", "Rossi", "m.rossi@example.com", "Mateo_Rossi_CV.pdf", PDF, "PENDING", 1, 11, None, ""),
    ("Yuki", "Tanaka", "yuki.tanaka@example.com", "Yuki_Tanaka_Resume.docx", DOCX, "PENDING", 2, 14, None, ""),
    ("Liang", "Wei", "liang.wei@example.com", "Liang_Wei_CV.pdf", PDF, "PENDING", 4, 16, None, ""),
    ("Priya", "Nair", "priya.nair@example.com", "Priya_Nair_Resume.pdf", PDF, "REACHED_OUT", 3, 9, 2,
     "Initial call done — gathering H-1B transfer docs. Strong candidate; following up Friday."),
    ("Sofia", "Ramírez", "sofia.ramirez@example.com", "Sofia_Ramirez_Resume.pdf", PDF, "REACHED_OUT", 5, 10, 4,
     "Left a voicemail + intro email. Awaiting reply; TN visa likely the fastest path."),
    ("Amara", "Okeke", "amara.okeke@example.com", "Amara_Okeke_Resume.docx", DOCX, "REACHED_OUT", 7, 13, 6,
...
def main() -> None:
    ...
    db.commit()
    try:
        listed = storage._internal.list_objects_v2(Bucket=storage.bucket, Prefix="resumes/")
    ...
    for first, last, email, fname, ctype, state, days, hour, reached_days, notes in PERSONAS:
        ...
        db.add(Lead(
            id=lead_id, first_name=first, last_name=last, email=email,
            resume_key=key, resume_filename=fname, resume_content_type=ctype,
            state=LeadState(state), notes=notes,
            created_at=created, updated_at=updated, reached_out_at=reached,
        ))
    db.commit()
